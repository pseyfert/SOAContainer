/** @file tests/SOAContainerDequeSimple.cc
 *
 * @brief some very basic SOA::Container tests, based on std::deque
 *
...
TEST(SOAContainerVectorSimple, WithSTLAlgorithms)
{
    ...
    // by comparing to an array-of-structures in a std::deque
    typedef std::size_t size_type;
    EXPECT_TRUE(c.empty());
    std::tuple<double, int, int> val(3.14, 0, 63);
    std::deque<std::tuple<double, int, int>> temp;
    for (int i = 0; i < 64; ++i) {
        std::get<1>(val) = i;
        std::get<2>(val) = 63 - i;
        temp.push_back(val);
