...
TEST(BasicTest, SimpleDequeTests) {
    SOA::Container<std::deque, SOA::PrintableNullSkin, double, int, int> c;
    const SOA::Container<std::deque, SOA::PrintableNullSkin, double, int, int>& cc = c;
    // check basic properties
    EXPECT_TRUE(c.empty());
    EXPECT_EQ(0u, c.size());
    c.clear();
...
namespace HitNamespaceDeque {
    ...
    using namespace Fields;
    typedef SOA::Container<std::deque, HitSkin, xAtYEq0, zAtYEq0, dxdy, dzdy, x, z, y> Hits;
    typedef typename Hits::reference Hit;
}

static void updateHits(HitNamespaceDeque::Hits& hits, float y0, float ySl) __attribute__((noinline));

static void updateHits(HitNamespaceDeque::Hits& hits, float y0, float ySl)
{
    using namespace HitNamespaceDeque;
    for (auto hit: hits) hit.updateHit(y0, ySl);
}

static void updateHits_v(HitNamespaceDeque::Hits& hits, float y0, float ySl) __attribute__((noinline));
static void updateHits_v(HitNamespaceDeque::Hits& hits, float y0, float ySl)
{
    using namespace HitNamespaceDeque;
    // give compiler a chance to not run out of registers, so it can
    // autovectorise
...
TEST(RealisticTest, SimpleDeque)
{
    using namespace HitNamespaceDeque;
    Hits hits;
    for (unsigned i = 0; i < 1024; ++i) {
